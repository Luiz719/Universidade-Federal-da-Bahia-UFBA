...
import { IntensityShader } 		from './shaders/IntensityShader.js';
...
const 	params = { enable : true, intensity : 1.0, gray: true, colorChannel: 3};
...
function main() {
	...
	gui.add( params, 'intensity' ).min(0.0).max(5.0).step(0.1).onChange(onChangeGUI);
...
function onLoadTexture(tex) {
	...
	if (!tex.image) 
		...
		const luminosity = new ShaderPass( IntensityShader );
		luminosity.uniforms['intensity'].value = params.intensity;
		luminosity.uniforms['colorChannel'].value = params.colorChannel;
