...
def realizar_experimento() -> dict[int, dict[int, float]] :
    ...
    for k in (2, 4, 16, 32):
        ...
        for m in (2, 4, 6, 8, 10, 12, 14):
            pm = texto[-m:]
            pms_valores.setdefault(m, tuple(bmh_match(texto, pm, k))[-1][1]/1000)
...
plt.title("Desempenho do algoritmo de busca em texto BMH")
