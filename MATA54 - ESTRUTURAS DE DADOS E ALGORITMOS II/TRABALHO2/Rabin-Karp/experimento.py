...
from algoritmo import rk_match
...
def gerar_alfabeto(k:int) -> list[str]:
    z = next(rk_match(string.printable, 'Z'))
...
def realizar_experimento() -> dict[int, dict[int, float]] :
    ...
    for k in (2, 4, 16, 32):
        ...
        for m in (2, 4, 6, 8, 10, 12, 14):
            pm = texto[-m:]
            pms_valores.setdefault(m, tuple(rk_match(texto, pm))[-1][1]/1000)
...
plt.title("Desempenho do algoritmo de busca em texto Rabin-Karp")
