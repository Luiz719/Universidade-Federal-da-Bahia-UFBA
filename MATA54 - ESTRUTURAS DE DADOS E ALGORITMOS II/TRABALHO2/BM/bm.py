    ...
    for i in range(2 + z[1], len(S)):
        ...
        if i <= r:  # i falls within existing z-box
            ...
            if b < a:  # b ends within existing z-box
                ...
                z[i], it = match_length(S, a, r + 1)
                z[i] += a
                iteracoes += it
                l = i
                r = i + z[i] - 1
        else:  # i does not reside within existing z-box
            z[i], it = match_length(S, 0, i)
            iteracoes += it
            if z[i] > 0:
                l = i
                r = i + z[i] - 1
...
def good_suffix_table(S: str) -> list[int]:
    ...
    """
    L = [-1 for c in S]
    N, it = fundamental_preprocess(S[::-1])  # S[::-1] reverses S
    N.reverse()
    iteracoes = it
...
def bm_match(T, P, alph_size) -> list[int]:
    """
    ...
    # Preprocessing
    R, itb = bad_character_table(P)
    L, itg = good_suffix_table(P)
    F, itf = full_shift_table(P)

    k = len(P) - 1      # Represents alignment of end of P relative to T
    previous_k = -1     # Represents alignment in previous phase (Galil's rule)
    iteracoes = itb + itg + itf + alph_size
    while k < len(T):
        iteracoes += 1
