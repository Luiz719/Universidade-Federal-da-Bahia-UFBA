...
struct expr *insert_expr(struct expr *head,
                         struct expr *elem) {
    struct expr *p = head;
    struct expr *i = elem;
    while (p->next) {
...
struct expr * create_boolean(int b)
{
    struct expr *ei =
        expression_create(EXPR_INTEGER_LITERAL,0,0,0,b);
    return ei;
}
...
struct expr * expr_create_boolean(int i)
{
    struct expr *eb =
        expr_create_integer(i);
    return eb;
}
